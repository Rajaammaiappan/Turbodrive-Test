...
def convert_pdf_to_excel_kofax(pdf_path, log=None, keep_converted=True, reuse=True):
    """
    Open `pdf_path` in Kofax Power PDF and drive the PDF → Excel conversion
    with keystrokes, saving the result next to the PDF in CONVERTED_SUBDIR
    (input.pdf → _converted/input.xlsx).

    Returns the converted .xlsx path, or None if the conversion failed.
    NOTE: this drives the real keyboard — do not use the PC while it runs.
    """
    def _log(m, l="info"):
        if log:
            log(m, l)
    ...
    out_dir = os.path.join(os.path.dirname(pdf_path), CONVERTED_SUBDIR) \
        if keep_converted else os.path.join(os.environ.get("TEMP", "."), "po_kofax")
    ...
    # 2 — Send the conversion keystrokes
    try:
        for keys, wait in KOFAX_KEYS:
            shell.SendKeys(keys)
            _log(f"  Sent keys: {keys}", "info")
            time.sleep(wait)
    except Exception as e:
        _log(f"  [ERROR] SendKeys failed: {e}", "error")
        return None

    # Some Kofax builds pop a "Convert Pages" dialog before the Save As
    try:
        conv = _find_window('Convert Pages', 5.0) or _find_window('Convert', 3.0) \
            or _find_window('Convert to Excel', 3.0)
        if conv:
            _activate_window(conv[0], conv[1])
            time.sleep(0.5)
            shell.SendKeys('{ENTER}')
            _log("  Confirmed Convert Pages dialog (pressed Enter)", "info")
            time.sleep(1.0)
    except Exception:
        pass

    # 3 — Save As dialog: type the output path and confirm
    if KOFAX_TYPE_PATH:
        save_win = _find_window('Save As', KOFAX_SAVE_WAIT) \
            or _find_window(os.path.basename(out_xlsx), KOFAX_SAVE_WAIT)
        try:
            if save_win:
                _activate_window(save_win[0], save_win[1])
                time.sleep(0.6)
                shell.SendKeys(_sk_escape(out_xlsx))
                time.sleep(0.5)
                shell.SendKeys("{ENTER}")
                _log(f"  Save As (dialog) → {out_xlsx}", "info")
            else:
                time.sleep(KOFAX_SAVE_WAIT)
                shell.SendKeys(_sk_escape(out_xlsx))
                time.sleep(0.8)
                shell.SendKeys("{ENTER}")
                _log(f"  Save As (fallback) → {out_xlsx}", "info")

            time.sleep(1.2)
            if KOFAX_OVERWRITE_KEYS:
                shell.SendKeys(KOFAX_OVERWRITE_KEYS)
        except Exception as e:
            _log(f"  [WARN] Could not type the Save As path: {e}", "warn")

    # 4 — Wait for the .xlsx to finish writing
    ok = _wait_for_stable_file(out_xlsx, KOFAX_CONVERT_TIMEOUT)

    # 5 — Close the document in Kofax
    try:
        for keys, wait in KOFAX_CLOSE_KEYS:
            shell.SendKeys(keys)
            time.sleep(wait)
    except Exception:
        pass
