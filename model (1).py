...
# ── TEMP DATA MASK ───────────────────────────────────────────────────────────
# When True, ideas whose category is in MASKED_CATEGORIES are excluded from
# every read path (dashboards, lists, kanban, charts, exports, counts) since
# they all flow through get_all(). Flip MASK_CATEGORY_DATA back to False (or
# clear MASKED_CATEGORIES) to restore full visibility — no other code needs
# to change.
MASK_CATEGORY_DATA = True
MASKED_CATEGORIES  = ["Customer Requirement", "Internal"]
...
def get_all():
    ...
    for r in (resp.data or []):
        if MASK_CATEGORY_DATA and r.get("category") in MASKED_CATEGORIES:
            continue
